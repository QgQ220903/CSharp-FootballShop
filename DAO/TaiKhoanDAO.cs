    ...
    public class TaiKhoanDAO : DatabaseAccess
    {
        ...
        public List<TaiKhoan> LayDanhSachTaiKhoan()
        {
            ...
            try
            {
                ...
                command = new SqlCommand("Select * from TaiKhoan", conn)
                {
        ...
        public List<TaiKhoan> TimKiemTaiKhoan(string text)
        {
            ...
            string sql = "select * from TaiKhoan where concat(MaTaiKhoan,TenTaiKhoan)COLLATE Latin1_General_CI_AI like N'%" + text + "%' and TrangThai = 1";
            command = new SqlCommand(sql, conn);
            OpenConnection();
            reader = command.ExecuteReader();
            while (reader.Read())
